    ...
    if float(b_first_price_apex)-float(s_first_price_dydx) > float(b_first_price_apex)*apex_make+float(s_first_price_dydx)*dydx_take:
          if arbitrage_count <8:
            ...
            task_apex_sell = asyncio.create_task(
                send_order_apex(client_apex, symbol="MATIC-USDC", side="SELL",
                                type="MARKET", size="30", expirationEpochSeconds=currentTime+1000,
                                price=0.1, limitFeeRate=limitFeeRate)
            )
            task_dydx_buy = asyncio.create_task(
                send_order_dydx(client_dydx, position_id, MARKET_MATIC_USD, ORDER_SIDE_BUY, ORDER_TYPE_LIMIT,
                                False, '30', s_first_price_dydx, '0.0015', currentTime+1000)
            )
    ...
    if float(b_first_price_dydx)-float(s_first_price_apex) > float(b_first_price_dydx)*dydx_take+float(s_first_price_apex)*apex_make:
      if arbitrage_count >-8:
        ...
        task_apex_buy = asyncio.create_task(
                send_order_apex(client_apex, symbol="MATIC-USDC", side="BUY",
                                type="MARKET", size="30", expirationEpochSeconds=currentTime+1000,
                                price=2, limitFeeRate=limitFeeRate)
            )
        task_dydx_sell = asyncio.create_task(
            send_order_dydx(client_dydx, position_id, MARKET_MATIC_USD, ORDER_SIDE_SELL, ORDER_TYPE_LIMIT,
                            False, '30', b_first_price_dydx, '0.0015', currentTime+1000)
        )
