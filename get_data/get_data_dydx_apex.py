...
# 确保保存数据的文件夹存在
output_dir = 'dydx&apex-depth-data'
...
async def fetch_and_save_orderbook(apex_client, dydx_client, symbol, market):
    ...
    while True:
        ...
        with open(apex_csv_path, mode='a', newline='') as apex_file:
            ...
            if apex_file.tell() == 0:
                apex_writer.writerow(['Price', 'Size', 'Type', 'Timestamp'])
        ...
        with open(dydx_csv_path, mode='a', newline='') as dydx_file:
            ...
            if dydx_file.tell() == 0:
                dydx_writer.writerow(['Price', 'Size', 'Type', 'Timestamp'])

            for ask in dydx_asks:
                dydx_writer.writerow([ask['price'], ask['size'], 'ask', timestamp])

            for bid in dydx_bids:
                dydx_writer.writerow([bid['price'], bid['size'], 'bid', timestamp])
